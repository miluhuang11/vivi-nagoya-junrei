...
>
> ⚠️ 資料庫本身是任何拿到網址的人都能新增/編輯/刪除的（沒有登入機制），
> 網站前端多加了一道「編輯密碼」門檻（見下方），但那只是擋一般人不小心亂改，
> 不是真正的安全機制，請不要放機密資料。
...
- 上方日期 pill 可以切換每一天，一次只顯示一天的行程。
- 打開網站當天如果剛好在行程日期範圍內，會自動跳到當天那一頁。
- 搜尋欄會跨全部天數搜尋景點/時間/備註，符合的那幾天都會顯示出來。
- 每張卡片標題／住宿資訊可以直接點下去編輯，點旁邊空白處會自動存檔。
- 點「＋ 新增」新增一個景點/行程項目，時間請用 24 小時制（例如 09:00）才能自動排序。
- 每個項目右邊有 ✏️ 編輯、🗑️ 刪除；點項目本身會開 Google 地圖。
- 第一次新增/編輯/刪除會要求輸入**編輯密碼**（目前是 `0619`，可在 `app.js` 的
  `EDIT_PIN` 改），輸入正確後這台裝置就不會再問。這只是簡單門檻，不是真正的帳號登入。
...
| `index.html` | 頁面結構 |
| `style.css` | 樣式 |
| `app.js` | 主要邏輯（讀寫 Supabase、即時同步、渲染畫面、編輯密碼） |
